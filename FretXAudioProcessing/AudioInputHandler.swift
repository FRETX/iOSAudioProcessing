//
//  AudioInputHandler.swift
//  FretX_prototype
//
//  Created by Onur Babacan on 24/05/2017.
...
class AudioInputHandler: NSObject {
    
    private(set) var audioUnit: AudioUnit!
    let audioSession : AVAudioSession = AVAudioSession.sharedInstance()
    var sampleRate: Float
    var numberOfChannels: Int
    var audioAnalyzers:[AudioAnalyzer] = []
    ...
    init(sampleRate: Float = 16000, numberOfChannels: Int = 1, bufferSizeInSeconds:Double = 0.05) {
        self.sampleRate = sampleRate
        self.numberOfChannels = numberOfChannels
        self.bufferSizeInSeconds = bufferSizeInSeconds
    }
    
    private func audioCallback(_ timeStamp: Double , _ numberOfFrames:Int, _ samples:[Float]){
//        print("nFrames: \(numberOfFrames)")
//        print("bufferSize: \(samples.count)")
        var audioData = AudioData(sampleRate: sampleRate, audioBuffer: samples)
        audioData.normalize()
        volume = 10 * log10(audioData.getSignalPower())
//        print("Volume: \(volume)")
        
        for analyzer in audioAnalyzers {
            analyzer.process(audioData: audioData)
//            print("sending data to \(analyzer)")
    ...
    private let recordingCallback: AURenderCallback = { (inRefCon, ioActionFlags, inTimeStamp, inBusNumber, inNumberFrames, ioData) -> OSStatus in
        ...
        let audioInput = unsafeBitCast(inRefCon, to: AudioInputHandler.self)
        var osErr: OSStatus = 0
        ...
        audioInput.audioCallback(inTimeStamp.pointee.mSampleTime / Double(audioInput.sampleRate),
                                      Int(inNumberFrames),
                                      monoSamples)
    ...
    private func setupAudioSession() {
        ...
        do {
            ...
            try audioSession.setPreferredIOBufferDuration(bufferSizeInSeconds)
